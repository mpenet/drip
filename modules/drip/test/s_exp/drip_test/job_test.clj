...
(deftest retry-policy-test
  (testing "default-retry-policy returns positive ms delay"
    (is (pos? (job/default-retry-policy 1))))

  (testing "default-retry-policy increases delay with attempt"
    (is (< (job/default-retry-policy 1) (job/default-retry-policy 5))))
  ...
  (testing "constant-retry-policy with jitter stays within expected range"
    (let [delay-ms 10000
          ...
          samples (repeatedly 20 #(policy 1))
          lo (long (* delay-ms (- 1 jitter)))
          hi (long (* delay-ms (+ 1 jitter)))]
  ...
  (testing "immediate-retry-policy returns 0"
